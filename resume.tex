...
\newenvironment{resumeentry}[5]{%
    ...
    \noindent\if\relax\detokenize{#3}\relax\else\textbf{#3}, \fi%
...
\begin{minipage}[t]{0.6\textwidth}
    ...
    {\small ML Engineer \& Tech Lead $\cdot$ Production ML \& LLM Reliability $\cdot$ 13+ yrs}\\[3pt]
...
\begin{projectdesc}
    Designed and shipped the first LLM assistant deployed to Zillow customer support: personalized Zestimate explanations grounded in real valuation data. Built RAG pipeline that constrains the agent to answer from actual Zestimate features and values rather than free-form generation; layered output validation with schema and range checks to catch out-of-distribution responses before they reach customers. Deployed with deflection rate tracked as the primary production KPI.\\[1pt]
...
\begin{projectdesc}
    Built Valuation API from POC to production: FastAPI service handling $\sim$6K requests/day with zero P2 alerts since launch. Led Zestimate 6.6 deprecation (Redis$\rightarrow$hybrid cache feature store for real-time valuations, legacy system shutdown), saving \$350K annually. Drove several Zestimate model point releases to completion; created MR tracking system, led ETL optimizations. Completed CI/CD pipeline modernization across all team services with zero production incidents.\\[1pt]
...
\begin{projectentry}
    \projicon{\faBrain} \textbf{Multimodal Embeddings \& Explainable Valuation} \tech{PyTorch, CLIP, Databricks} \approach{DualLossAutoEncoder + Per-feature Attribution}
\end{projectentry}
...
\begin{projectdesc}
    Owned end-to-end ML pipeline for subscription discount optimization: feature engineering, model training, A/B testing, deployment, and production monitoring. Discovered high prediction variance across model runs; designed ensemble uncertainty estimation using 100-model bagging to quantify and stabilize outputs for production deployment.\\[1pt]
...
\textbf{ML/AI} & LangChain $\cdot$ RAG $\cdot$ LLM Guardrails $\cdot$ Embeddings $\cdot$ CLIP $\cdot$ PyTorch $\cdot$ TensorFlow $\cdot$ A/B Testing $\cdot$ Uncertainty Estimation $\cdot$ Monitoring \\
